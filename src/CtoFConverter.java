...
public class CtoFConverter {
    public static void main(String[] args) {
        ...
        double cTemp = 0.0; //temperature in Celcius to be entered
        double fTemp = 0.0; //teperature in Farenheit that will be calculated
        boolean done = false;
        String trash = "";
        String continueYN = "";
        ...
        try (Scanner in = new Scanner(System.in)) { // try auto-closes Scanner so there's no memory leak
            ...
            do {
                System.out.print("\nEnter a temperature value in Celcius: ");
                if(in.hasNextDouble()) { //safe to read in a double
                    cTemp = in.nextDouble();
                    in.nextLine(); //clears the newline from buffer
                    fTemp = 1.8 * cTemp + 32;
                    System.out.printf("%n%.2f degrees Celcius is equal to %.2f degrees Farenheit.", cTemp, fTemp);
                    ...
                    trash = in.nextLine();
                    System.out.println("\nInvalid input. Please enter a number instead.");
                }
            } while(!done);
        }
    }
